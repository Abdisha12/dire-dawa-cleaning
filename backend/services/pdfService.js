...
function fmtETB(n) {
  return (
    "ETB " +
    parseFloat(n || 0).toLocaleString("en-ET", {
...
function drawHeader(doc, title, metadata = {}) {
  ...
  doc.fillColor("#1e3a8a").font("Helvetica-Bold").fontSize(16).text("Dire Dawa City Administration", 40, 55);

  doc.fillColor("#4b5563").font("Helvetica").fontSize(11).text("Cleaning Management System - Official Report", 40, 75);

  doc.fillColor("#0d9488").font("Helvetica-Bold").fontSize(13).text(title, 40, 95);

  doc.strokeColor("#e2e8f0").lineWidth(1).moveTo(40, 112).lineTo(555, 112).stroke();
  ...
  if (metadata.month && metadata.year) {
    doc.text(`REPORT PERIOD: ${monthName(metadata.month).toUpperCase()} ${metadata.year}`, 380, metaY, {
      align: "right",
      width: 175
    });
    metaY += 12;
...
function generatePaymentsPDF(doc, data, month, year) {
  ...
  const rowData = data.map((p) => [p.business, p.zone, p.kebele, p.amount, p.method, p.paid_at, p.receipt_number]);
...
function generateInspectionsPDF(doc, data, from, to) {
  ...
  const poorCount = data.filter(
    (i) => (i.status || "").toLowerCase() === "poor" || (i.status || "").toLowerCase() === "broken"
  ).length;
  ...
  const rowData = data.map((i) => [i.date, i.kebele, i.zone, i.status, i.inspector, i.notes]);
