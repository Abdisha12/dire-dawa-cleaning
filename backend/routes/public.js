...
router.get("/stats",async(req,res,next)=>{
  try{
    ...
    const inspectionsResult=await db.query("SELECT COUNT(*) AS c FROM inspections WHERE date>=NOW() - INTERVAL '30 days'");
    const collectedResult=await db.query("SELECT COALESCE(SUM(amount),0) AS total FROM payments WHERE status='paid' AND month=$1 AND year=$2",[m,y]);
