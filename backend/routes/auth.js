...
async function recordFailedLogin(username, ipAddress) {
  const now = new Date();
  // Check existing attempt for this username+ip
  const existing = await db.query(
    "SELECT failed_count, last_attempt_at, locked_until FROM login_attempts WHERE username=$1 AND ip_address=$2",
    [username, ipAddress]
  );
  if (existing.rows.length > 0) {
    const row = existing.rows[0];
    // If currently locked and past lockout, reset
    if (row.locked_until && row.locked_until > now) {
      // Still locked - increment but keep locked_until
      await db.query(
        "UPDATE login_attempts SET failed_count=$1, last_attempt_at=$2 WHERE id=$3",
        [row.failed_count + 1, now, row.id]
      );
    } else {
      // Not locked or lockout expired - reset count and locked_until
      await db.query(
        "UPDATE login_attempts SET failed_count=1, last_attempt_at=$2, locked_until=NULL WHERE id=$1",
    ...
    await db.query(
      "INSERT INTO login_attempts (username, ip_address, failed_count, last_attempt_at) VALUES ($1, $2, 1, $3)",
...
async function isLocked(username, ipAddress) {
  ...
  const result = await db.query(
    "SELECT failed_count, locked_until FROM login_attempts WHERE username=$1 AND ip_address=$2",
    [username, ipAddress]
  );
...
router.post("/login", validate(schemas.login), async (req, res, next) => {
  try {
    ...
    if (await isLocked(username, clientIp)) {
      ...
      // Return uniform response - do not leak lockout state
