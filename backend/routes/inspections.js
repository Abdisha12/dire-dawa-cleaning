...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    const d = path.join(__dirname, "../uploads/inspections");
  ...
  filename: (req, file, cb) =>
    cb(
      null,
      `insp_${Date.now()}_${crypto.randomBytes(6).toString("hex")}${path.extname(file.originalname).toLowerCase()}`
    )
});
...
router.get("/", validate(schemas.inspectionListQuery, "query"), async (req, res, next) => {
  try {
    ...
    if (req.user.role === "leader") {
      sqlBase += ` AND sz.leader_id=$${idxBase}`;
      paramsBase.push(req.user.id);
      idxBase++;
      ...
      sqlBase += ` AND i.kebele_id=$${idxBase}`;
      paramsBase.push(assignedKebele);
      idxBase++;
      if (zoneId) {
        sqlBase += ` AND i.safer_zone_id=$${idxBase}`;
        paramsBase.push(zoneId);
        idxBase++;
      }
    ...
    if (status) {
      sqlBase += ` AND i.status=$${idxBase}`;
      paramsBase.push(status);
      idxBase++;
    }
    if (from) {
      sqlBase += ` AND i.date>=$${idxBase}`;
  ...
  try {
    const result = await db.query(
      `SELECT i.*,k.name AS kebele_name,sz.name AS zone_name,u.full_name AS inspector_name
       ...
       JOIN users u ON u.id=i.inspected_by WHERE i.id=$1`,
      [req.params.id]
    );
  ...
  async (req, res, next) => {
    try {
      ...
      if (req.user.role === "collector") {
        ...
        if (String(kebeleId) !== String(assignedKebele))
          return res.status(403).json({ error: "Kebele does not match your assigned kebele" });
        if (saferZoneId) {
          const zc = await db.query("SELECT id FROM safer_zones WHERE id=$1 AND kebele_id=$2", [
            saferZoneId,
            assignedKebele
          ]);
      ...
      if (req.files?.length) {
        for (const f of req.files)
          await db.query("INSERT INTO inspection_photos (inspection_id,file_path) VALUES ($1,$2)", [
            insertedId,
            `/uploads/inspections/${f.filename}`
          ]);
      }
      audit.log(req, "CREATE", "inspection", insertedId, null, {
        kebeleId,
        saferZoneId,
        date,
...
router.delete("/:id", requireRole("admin", "collector"), async (req, res, next) => {
  try {
    const oldResult = await db.query("SELECT kebele_id,safer_zone_id,date,status FROM inspections WHERE id=$1", [
      req.params.id
    ]);
    const photosResult = await db.query("SELECT file_path FROM inspection_photos WHERE inspection_id=$1", [
      req.params.id
    ]);
    const uploadsDir = path.resolve(__dirname, "../uploads/inspections");
