...
router.get("/", validate(schemas.zoneReportListQuery, "query"), async (req, res, next) => {
  try {
    ...
    if (req.user.role === "leader") {
      ...
      whereParams.push(assignedKebele);
      wIdx++;
      if (zoneId) {
        baseSql += ` AND zr.safer_zone_id=$${wIdx}`;
    ...
    if (month) {
      baseSql += ` AND zr.report_month=$${wIdx}`;
      whereParams.push(month);
      wIdx++;
    }
    if (year) {
      baseSql += ` AND zr.report_year=$${wIdx}`;
      whereParams.push(year);
      wIdx++;
    }
    if (status) {
      baseSql += ` AND zr.status=$${wIdx}`;
  ...
  try {
    const result = await db.query(
      `SELECT zr.*,sz.name AS zone_name,k.name AS kebele_name,
       ...
       LEFT JOIN users r ON r.id=zr.reviewed_by WHERE zr.id=$1`,
      [req.params.id]
    );
...
router.post(
  ...
  validate(schemas.createZoneReport),
  async (req, res, next) => {
    try {
      const {
        saferZoneId,
      ...
      audit.log(req, "CREATE", "zone_report", insertedId, null, {
        saferZoneId,
        reportDate,
        status: "draft",
        workersPresent,
      ...
      if (err.code === "23505")
        return res.status(409).json({ error: "A report already exists for this zone/month/year" });
      next(err);
    }
  }
);
...
router.put(
  "/:id",
  requireRole("admin", "collector", "leader"),
  validate(schemas.updateZoneReport),
  async (req, res, next) => {
    try {
      const { workersPresent, workersAbsent, collectionTotal, issuesReported, actionsTaken, toolsStatus, status } =
        req.body;
      if (req.user.role === "leader") {
        const check = await db.query(
          `SELECT zr.id FROM zone_reports zr JOIN safer_zones sz ON sz.id=zr.safer_zone_id
         WHERE zr.id=$1 AND sz.leader_id=$2`,
          [req.params.id, req.user.id]
        );
        if (!check.rows.length) return res.status(403).json({ error: "Not your zone's report" });
      }

      const currentResult = await db.query("SELECT * FROM zone_reports WHERE id=$1", [req.params.id]);
      ...
      if (status && status !== current.status) {
        ...
        if (allowedRoles && !allowedRoles.includes(req.user.role)) {
          return res
            .status(403)
            .json({ error: `Role '${req.user.role}' cannot transition from ${current.status} to ${status}` });
        }
        newStatus = status;
      }

      const wp = workersPresent !== undefined ? workersPresent : current.workers_present;
      const wa = workersAbsent !== undefined ? workersAbsent : current.workers_absent;
      const ct = collectionTotal !== undefined ? collectionTotal : current.collection_total;
      const ir = issuesReported !== undefined ? issuesReported : current.issues_reported;
      const at = actionsTaken !== undefined ? actionsTaken : current.actions_taken;
      const ts = toolsStatus !== undefined ? toolsStatus : current.tools_status;
      ...
      await db.query(
       ...
       issues_reported=$4,actions_taken=$5,tools_status=$6,status=$7 WHERE id=$8`,
        [wp, wa, ct, ir, at, ts, newStatus, req.params.id]
      );
      audit.log(
        req,
        "UPDATE",
        ...
        { workersPresent, workersAbsent, collectionTotal, status: newStatus }
      );
...
router.put(
  ...
  validate(schemas.reviewZoneReport),
  async (req, res, next) => {
    try {
...
router.delete("/:id", requireRole("admin"), async (req, res, next) => {
  try {
    const oldResult = await db.query("SELECT safer_zone_id,report_date,status FROM zone_reports WHERE id=$1", [
      req.params.id
    ]);
