...
router.put("/:id", requireRole("admin"), validate(schemas.updateUser), async (req, res, next) => {
  try {
    ...
    const oldResult = await db.query("SELECT full_name,fayda_id,phone,role,is_active FROM users WHERE id=$1", [
      req.params.id
    ]);
    ...
    audit.log(req, "UPDATE", "user", parseInt(req.params.id), oldResult.rows[0] || null, {
      fullName,
      faydaId,
      phone,
      role,
