...
router.use(["/kebeles", "/safer-zones", "/businesses"], authenticate);
  ...
  try {
    const result = await db.query(
      `SELECT k.*,u.full_name AS collector_name,u.phone AS collector_phone,
       ...
       GROUP BY k.id, u.id ORDER BY k.code`
    );
  ...
  try {
    const result = await db.query(
      `SELECT sz.*,k.name AS kebele_name,k.code AS kebele_code,
              u.full_name AS leader_name,u.phone AS leader_phone
       FROM safer_zones sz JOIN kebeles k ON k.id=sz.kebele_id
       LEFT JOIN users u ON u.id=sz.leader_id WHERE sz.id=$1`,
      [req.params.id]
    );
...
router.delete("/safer-zones/:id", requireRole("admin"), async (req, res, next) => {
  try {
    await db.query("DELETE FROM safer_zones WHERE id=$1", [req.params.id]);
...
router.get("/businesses", async (req, res, next) => {
  try {
    ...
    if (req.user.role === "leader") {
      ...
      wIdx++;
      if (saferZoneId) {
        baseSql += ` AND b.safer_zone_id=$${wIdx}`;
        whereParams.push(parseInt(String(saferZoneId), 10));
        wIdx++;
      }
      ...
      if (kebeleId) {
        baseSql += ` AND k.id=$${wIdx}`;
        whereParams.push(parseInt(String(kebeleId), 10));
        wIdx++;
      }
    }
    if (search) {
...
router.post(
  ...
  validate(schemas.createBusiness),
  async (req, res, next) => {
    try {
      const { name, ownerName, ownerFaydaId, ownerPhone, type, monthlyTarget, saferZoneId, notes } = req.body;
      if (!name || !ownerName || !saferZoneId)
        return res.status(400).json({ error: "name,ownerName,saferZoneId required" });
      if (req.user.role === "collector") {
        const kebeleRes = await db.query("SELECT id FROM kebeles WHERE collector_id=$1", [req.user.id]);
        const assignedKebele = kebeleRes.rows[0]?.id || null;
        if (!assignedKebele) return res.status(403).json({ error: "No kebele assigned to your account" });
        const zoneCheck = await db.query("SELECT id FROM safer_zones WHERE id=$1 AND kebele_id=$2", [
          saferZoneId,
          assignedKebele
        ]);
      ...
      if (req.user.role === "collector") {
        ...
        if (assignedKebele) {
          ...
          const cur = await db.query(
            "SELECT k.id AS kebele_id FROM businesses b JOIN safer_zones sz ON sz.id=b.safer_zone_id JOIN kebeles k ON k.id=sz.kebele_id WHERE b.id=$1",
            [req.params.id]
          );
          if (cur.rows.length && cur.rows[0].kebele_id !== assignedKebele)
            return res.status(403).json({ error: "Business not in your kebele" });
          if (saferZoneId) {
            const zoneCheck = await db.query("SELECT id FROM safer_zones WHERE id=$1 AND kebele_id=$2", [
              saferZoneId,
              assignedKebele
            ...
            if (!zoneCheck.rows.length)
              return res.status(403).json({ error: "Cannot move business to another kebele" });
          }
        }
      }
