...
router.get("/summary/dashboard", validate(schemas.dashboardQuery, "query"), async (req, res, next) => {
  try {
    ...
    if (req.user.role === "leader") {
      whereExtra = ` AND sz.leader_id=$${extraIdx}`;
    ...
    const totalsResult = await db.query(
      `SELECT SUM(CASE WHEN p.status='paid' THEN p.amount ELSE 0 END) AS total_collected,
       ...
       WHERE p.year=$1 AND p.month=$2${whereExtra}`,
      [y, m, ...extraParams]
    );
    ...
    let whereExtra2 = "",
      ...
      extraIdx2 = 3;
    if (req.user.role === "leader") {
      whereExtra2 = ` AND sz.leader_id=$${extraIdx2}`;
      extraParams2 = [req.user.id];
    }
    ...
    let whereExtra3 = "",
      ...
      extraIdx3 = 2;
    if (req.user.role === "leader") {
      whereExtra3 = ` AND sz.leader_id=$${extraIdx3}`;
      extraParams3 = [req.user.id];
    }
...
router.get("/", validate(schemas.paymentsListQuery, "query"), async (req, res, next) => {
  try {
    ...
    if (req.user.role === "leader") {
      ...
      if (bizId) {
        baseSql += ` AND p.business_id=$${wIdx}`;
        whereParams.push(bizId);
        wIdx++;
      }
      if (kebId) {
        baseSql += ` AND k.id=$${wIdx}`;
        whereParams.push(kebId);
        wIdx++;
      }
      if (szId) {
        baseSql += ` AND sz.id=$${wIdx}`;
        whereParams.push(szId);
        wIdx++;
      }
    }
    if (month) {
      baseSql += ` AND p.month=$${wIdx}`;
      whereParams.push(month);
      wIdx++;
    }
    if (year) {
      baseSql += ` AND p.year=$${wIdx}`;
      whereParams.push(year);
      wIdx++;
    }
    if (status) {
      baseSql += ` AND p.status=$${wIdx}`;
      whereParams.push(status);
      wIdx++;
    }
    if (method) {
      baseSql += ` AND p.method=$${wIdx}`;
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      if (!businessId || !amount || !month || !year)
        return res.status(400).json({ error: "businessId,amount,month,year required" });
      ...
      audit.log(req, "CREATE", "payment", insertedId, null, {
        businessId,
        amount,
        method: method || "cash",
        month,
...
router.put("/:id", requireRole("admin", "collector"), validate(schemas.updatePayment), async (req, res, next) => {
  try {
    ...
    audit.log(req, "UPDATE", "payment", parseInt(req.params.id), oldResult.rows[0] || null, {
      amount,
      method,
      status,
      notes
...
router.delete("/:id", requireRole("admin"), async (req, res, next) => {
  try {
    const oldResult = await db.query("SELECT business_id,amount,method,status,month,year FROM payments WHERE id=$1", [
      req.params.id
    ]);
