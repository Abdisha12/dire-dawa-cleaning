...
router.get("/safer-zones", async (req, res, next) => {
  try {
    ...
    const sql = `SELECT sz.id,sz.name,k.code AS code,k.name AS kebele_name,k.code AS kebele_code,
    ...
    const nullFeatures = result.rows
      ...
      .map((r) => ({
        ...
        properties: {
          ...
          name: r.name,
          code: r.code,
          kebeleName: r.kebele_name,
          kebeleCode: r.kebele_code,
...
router.get("/businesses", async (req, res, next) => {
  try {
    ...
    if (req.user.role === "leader") {
      ...
      if (parsedKebele.value !== undefined) {
        baseSql += ` AND k.id=$${idx}`;
        whereParams.push(parsedKebele.value);
        idx++;
      }
    }
    if (search) {
  ...
  switch (entityType) {
    case "Business":
      return {
        ...
        saferZoneId: row.safer_zone_id,
        saferZoneName: row.safer_zone_name,
