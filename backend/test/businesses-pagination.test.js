...
describe("Businesses & Payments Pagination", function () {
  ...
  before(async function () {
    ...
    for (const [key, u] of Object.entries({ admin: userIds.admin, collector: userIds.collector, viewer: userIds.viewer })) {
      const token = uuidv4();
  ...
  describe("GET /api/businesses — pagination & search", function () {
    it("returns paginated envelope when page+limit provided", async function () {
      const res = await request(app).get("/api/businesses").query({ page: 1, limit: 10 }).set("x-session-token", tokens.admin);
    ...
    it("search filters by name (ILIKE)", async function () {
      ...
      const createRes = await request(app).post("/api/businesses").set("x-session-token", tokens.admin).send({ name: bizName, ownerName: "Owner X", saferZoneId: testZone1.id, type: "shop" });
      expect(createRes.status).to.equal(201);
      const res = await request(app).get("/api/businesses").query({ page: 1, limit: 10, search: bizName }).set("x-session-token", tokens.admin);
      expect(res.status).to.equal(200);
    ...
    it("type filter works", async function () {
      const res = await request(app).get("/api/businesses").query({ page: 1, limit: 50, type: "shop" }).set("x-session-token", tokens.admin);
      expect(res.status).to.equal(200);
  ...
  describe("GET /api/payments — pagination & search", function () {
    it("returns paginated envelope when page+limit provided", async function () {
      const res = await request(app).get("/api/payments").query({ page: 1, limit: 10 }).set("x-session-token", tokens.admin);
      expect(res.status).to.equal(200);
      expect(res.body).to.be.an("object");
      expect(res.body).to.have.property("data").that.is.an("array");
      expect(res.body.total).to.be.a("number");
