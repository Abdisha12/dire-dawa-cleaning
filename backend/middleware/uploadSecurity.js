...
const ALLOWED = {
  ...
  inspection: {
    extensions: new Set([".png", ".jpg", ".jpeg", ".gif", ".webp", ".bmp"]),
    mimes: new Set(["image/png", "image/jpeg", "image/gif", "image/webp", "image/bmp"]),
...
function validateUploadedFile(context) {
  ...
  return function postUploadValidation(req, res, next) {
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (!textExts.has(ext)) {
          ...
          if (context === "document" && !config.mimes.has(detectedMime) && detectedMime !== "application/zip") {
            ...
            return res
              .status(400)
              .json({ error: `File content does not match allowed types (detected: ${detectedMime}).` });
          }
        }
      } catch (err) {
      ...
      const uploadsDir = require("path").resolve(
        __dirname,
        `../uploads/${context === "inspection" ? "inspections" : "documents"}`
      );
