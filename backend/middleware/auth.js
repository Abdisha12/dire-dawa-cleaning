  ...
  try{
    ...
    const result=await db.query(
      `SELECT s.id AS sid, s.user_id, u.id, u.username, u.full_name, u.role, u.is_active
       ...
       WHERE s.id=$1 AND s.expires_at>NOW() AND u.is_active=TRUE`,[token]);
...
async function zoneAccess(req,res,next){
  ...
  if(req.user.role==="leader"){
    ...
    const result=await db.query("SELECT id FROM safer_zones WHERE id=$1 AND leader_id=$2",[zoneId,req.user.id]);
    if(!result.rows.length) return res.status(403).json({error:"You can only access your own zone"});
  }
  next();
}
